...
${modifyHPDeskReq } --%>
			 <h2 class="title1">문의사항 수정(일반사원)</h2>
			 <%-- 일반사원은 자신의 글의 제목과 내용만 수정가능 --%>
			 <form name="insertFrm" id="insertFrm" method="post" action="<%=request.getContextPath() %>/hpdesk/modifyhpdesk.aa?hdno=${modifyHPDeskReq.hdno}">
	    	...
	    	<tr>
	       	<th class="thead">게시글제목</th>
	       		<td class="tbody">
	       				<input class="texta"type="text" name="hdtitle" id="hdtitle" autofocus="autofocus" value="${modifyHPDeskReq.hdtitle }"/>
	       		</td>
	    	</tr>
	    	<tr>
	       	<th class="thead">게시글내용</th>
	       		<td class="tbody">
	       				<pre><textarea class="texta"name="hdcontent" id="hdcontent" rows="20" cols="70">${modifyHPDeskReq.hdcontent }</textarea></pre>
	       		</td>
	    	</tr>
				<%-- 일반유저모드에서는 보이지 않음 --%>
	    	<tr class="t1">
       	<th>게시글댓글</th>
	       		 <td>
	       		 		<textarea name="hdcomment" id="hdcomment" rows="10" cols="70">${modifyHPDeskReq.hdcomment }</textarea>	
	       		 </td>	       		 
	    	</tr>
	    	<tr class="t1">
				    <th >글확인상태</th>
						<td colspan="2" style="text-align:right;">	
	 									<select name="hdcheck"  id="hdcheck">
				...
				<c:if test="${EMP_USER.empno==modifyHPDeskReq.empno}">
				<br/>

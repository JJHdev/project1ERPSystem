...
public class MypageDeletePage {

	//필드
	private int total;			 //전체게시물수
	private int currentPage;	 //현재 페이지(요청페이지)
	private List<MypageEmp> content;//게시글목록정보(글번호,제목,작성자,조회수,최초작성일,마지막수정일,노출여부)
	...
	public MypageDeletePage(int total, int currentPage, 
					   int size, List<MypageEmp> content) {
		this.total = total;
	...
	//게시글없니? 게시글없으면 true리턴
	public boolean hasNoMypageDelete() {
		return total == 0;
	}

	//게시글이 존재하니? 게시글 있으면 true
	public boolean hasMypageDelete() {
		return total > 0;
	}
	...
	public List<MypageEmp> getContent() {
		return content;
	}

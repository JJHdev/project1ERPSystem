...
public class MypageUpdateLevelController implements CommandController {

	private static final String FORM_VIEW = "/view/updatemypage/changeLevelForm.jsp";
	...
	//비번변경폼으로 이동-p623 32라인
		...
		private String processSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			//1.파라미터받기
			String curLevel = trim(request.getParameter("curLevel")); //현재 비번(변경전비번)
			String newLevel = trim(request.getParameter("newLevel")); //new 비번(변경할비번)
			String strempNo  = request.getParameter("empno");
			int empno = Integer.parseInt(strempNo);
			...
			if(curLevel==null || curLevel.isEmpty()) {
				errors.put("curLevel", Boolean.TRUE);
			}
			if(newLevel==null || newLevel.isEmpty()) {
				errors.put("newLevel", Boolean.TRUE);
			}
			...
			try {
				updateService.updateLevel(empno, curLevel, newLevel);
				request.setAttribute("empno",empno);
				return mypageInfoController.searchView(request, response,empno);
			}catch(InvalidException e) {
				errors.put("badCurLevel", Boolean.TRUE);
				request.setAttribute("empno",empno);
				return FORM_VIEW;

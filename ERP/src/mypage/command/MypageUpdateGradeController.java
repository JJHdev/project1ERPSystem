...
public class MypageUpdateGradeController implements CommandController {

	private static final String FORM_VIEW = "/view/updatemypage/changeGradeForm.jsp";
	...
	//비번변경폼으로 이동-p623 32라인
		...
		private String processSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			//1.파라미터받기
			String strcurGrade = trim(request.getParameter("curGrade")); //현재 비번(변경전비번)
			String strnewGrade = trim(request.getParameter("newGrade")); //new 비번(변경할비번)
			String strempNo  = request.getParameter("empno");
			int empno = Integer.parseInt(strempNo);
			...
			if(strcurGrade==null || strcurGrade.isEmpty()) {
				errors.put("curGrade", Boolean.TRUE);
			}
			if(strnewGrade==null || strnewGrade.isEmpty()) {
				errors.put("newGrade", Boolean.TRUE);
			}
			//p623 53라인
			...
			try {
				int curGrade = Integer.parseInt(strcurGrade);
				int newGrade = Integer.parseInt(strnewGrade);
				updateService.updateGrade(empno, curGrade, newGrade);
				request.setAttribute("empno",empno);
				return mypageInfoController.searchView(request, response,empno);
			}catch(InvalidException e) {
				errors.put("badCurGrade", Boolean.TRUE);
				request.setAttribute("empno",empno);
				return FORM_VIEW;
				...
				request.setAttribute("empno",empno);
				errors.put("StringCurGrade", Boolean.TRUE);
				return FORM_VIEW;
			}
		}

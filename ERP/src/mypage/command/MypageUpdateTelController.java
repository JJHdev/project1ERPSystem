...
public class MypageUpdateTelController implements CommandController {
	
	private static final String FORM_VIEW = "/view/updatemypage/changeTelForm.jsp";
	...
	//비번변경폼으로 이동-p623 32라인
		...
		private String processSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			//1.파라미터받기
			String curTel = trim(request.getParameter("curTel")); //현재 비번(변경전비번)
			String newTel = trim(request.getParameter("newTel")); //new 비번(변경할비번)
			String strempNo  = request.getParameter("empno");
			int empno = Integer.parseInt(strempNo);
			...
			if(curTel==null || curTel.isEmpty()) {
				errors.put("curTel", Boolean.TRUE);
			}
			...
			try {
				updateService.updateTel(empno, curTel, newTel);
				request.setAttribute("empno",empno);
				return mypageInfoController.searchView(request, response,empno);
			}catch(InvalidException e) {
				request.setAttribute("empno",empno);
				errors.put("badCurTel", Boolean.TRUE);
				return FORM_VIEW;

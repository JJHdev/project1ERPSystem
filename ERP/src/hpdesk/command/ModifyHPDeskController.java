...
//글내용 수정(일반사원)
//http://localhost/hpdesk/modifyhpdesk.aa
public class ModifyHPDeskController implements CommandController {
	private static final String FORM_VIEW="/view/hpdesk/modiyHPDeskForm.jsp";
	//상세보기 서비스
	private ReadHPDeskService readHPDeskService=new ReadHPDeskService();
	//수정처리 서비스
	private ModifyHPDeskService modifyHPDeskService =new ModifyHPDeskService();
	@Override
	...
	private String processForm(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			String hdtitle =request.getParameter("hdtitle");//수정할 제목
			String hdcontent =request.getParameter("hdcontent");//수정할 내용
			/*파라미터 int hdno : 상세조회할 글번호
			 *리턴유형 HPDeskData: hpdesk테이블과 hdcomm테이블 관련 데이터  */  
		...
		private String processSubmit(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			modifyHPDeskService.modifyHPDesk(modifyHPDeskReq);

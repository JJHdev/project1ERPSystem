...
public class SurvResultPage {

	//필드
	private int total; //전체게시물수
	private int currentPage;//현재 페이지(요청페이지)
	private List<SurvResult> content;//게시글목록정보(글번호,제목,참여자수,최초작성일,마지막수정일,노출여부)
	...
	public SurvResultPage(int total, int currentPage, 
					int size, List<SurvResult> content) {
					this.total = total;
	...
	public List<SurvResult> getContent() {
		return content;
	}
	...
	//게시글없니? 게시글없으면 true리턴
		...
		public String toString() {
			return "SurvResultPage [total=" + total + ", currentPage=" + currentPage + ", content=" + content
